...
void Dictionary::readFromFile(
  ...
  std::cerr << "Number of words (" << args_->k << "-mers) in dictionary:  " << nwords_ << std::endl;
...
void Dictionary::addKmers(unsigned k, int64_t &minThreshold, unsigned len = 0,
                          string seq = "", string rc_seq = ""){
  if(len == k){
    int32_t h = find(rc_seq);
    if(hashToIndex_[h] == -1){
      insert(seq);
      if(size_ > 0.75 * MAX_VOCAB_SIZE){
        minThreshold++;
    ...
    addKmers(k, minThreshold, len + 1, seq + "A", "T" + rc_seq);
    addKmers(k, minThreshold, len + 1, seq + "C", "G" + rc_seq);
    addKmers(k, minThreshold, len + 1, seq + "T", "A" + rc_seq);
    addKmers(k, minThreshold, len + 1, seq + "G", "C" + rc_seq);
  }
}

void Dictionary::addCells(unsigned nCells, int64_t &minThreshold, string label){
  for (unsigned ci = 0; ci < nCells; ci ++) {
    insert(label + "C" + to_string(ci + 1));
    if (size_ > 0.75 * MAX_VOCAB_SIZE) {
      minThreshold++;
      threshold(minThreshold, minThreshold);
    }
...
void Dictionary::addBatches(unsigned nBatches, int64_t &minThreshold, string label){
  for (unsigned bi = 0; bi < nBatches; bi ++) {
    insert(label + "B" + to_string(bi + 1));
    if (size_ > 0.75 * MAX_VOCAB_SIZE) {
      minThreshold++;
      threshold(minThreshold, minThreshold);
    }
...
void Dictionary::CreateForCellSpace(shared_ptr<DataParser> parser){
  int64_t minThreshold = 1;
  addKmers(args_->k, minThreshold); // Add k-mer IDs as words
  addCells(args_->nCells_total, minThreshold, args_->label); // Add cell IDs as labels
//  addPeaks(args_->nPeaks_total, minThreshold, args_->label); // Add peak IDs as labels
  // if(args_->batchLabels) addBatches(args_->nBatches, minThreshold, args_->label); // Add batch IDs as labels

  threshold(args_->minCount, args_->minCountLabel);

//  std::cerr << "\rRead " << ntokens_  / 1000000 << "M words" << std::endl;
  std::cout << "Number of words (" << args_->k << "-mers) in dictionary:  " << nwords_ << std::endl
            << "Number of labels in dictionary: " << nlabels_ << std::endl;
}
