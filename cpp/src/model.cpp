...
float EmbedModel::trainOneBatch(shared_ptr<InternDataHandler> data,
  ...
  size_t num_datasets = args_->nCells_list.size();
  std::vector<std::vector<Matrix<Real>>> rhsN(num_datasets, std::vector<Matrix<Real>>(negSearchLimit));
  std::vector<std::vector<std::vector<Base>>> batch_negLabels(num_datasets);

  for(unsigned dataset = 0; dataset < num_datasets; dataset++){
    // std::cerr << "dataset " << dataset + 1 << std::endl;
    for(unsigned int j = 0; j < negSearchLimit; j++) {
      std::vector<Base> negLabels;
      if (trainWord) {
        data->getRandomWord(negLabels);
      } else {
        data->getRandomRHS(negLabels, dataset);
      }
      projectRHS(negLabels, rhsN[dataset][j]);;
      check(rhsN[dataset][j]);
      batch_negLabels[dataset].push_back(negLabels);
    }
    // std::cerr << std::endl;
  }
  ...
  std::vector<unsigned> dataset(batch_sz);
  ...
  for (int i = 0; i < batch_sz; i++) {
    ...
    dataset[i] = batch_exs[i].dataset;

// for(auto word: batch_exs[i].RHSTokens) std::cerr << std::endl << dict_->getSymbol(word.first) << ": ";

    for (unsigned int j = 0; j < negSearchLimit; j++) {
      nRate[i][j] = 0.0;
      if(batch_exs[i].RHSTokens == batch_negLabels[dataset[i]][j]) continue;

// auto word = batch_negLabels[dataset[i]][j][0];
// cerr << dict_->getSymbol(word.first) << " - ";

      auto thisLoss = tripleLoss(posSim[i], similarity(lhs[i], rhsN[dataset[i]][j]));
      if (thisLoss > 0.0) {
        num_negs[i]++;
        loss[i] += thisLoss;
        negMean[i].add(rhsN[dataset[i]][j]);
        assert(loss[i] >= 0.0);
        update_flag[i][j] = true;
        if (num_negs[i] == args_->maxNegSamples) break;
      }
    }
...
void EmbedModel::backward(
    const vector<ParseResults>& batch_exs,
    const vector<vector<vector<Base>>>& batch_negLabels,
  ...
  for (unsigned int i = 0; i < batch_sz; i++){
    unsigned d = dataset[i];
    for (unsigned int j = 0; j < batch_negLabels[d].size(); j++) {
      if (fabs(nRate[i][j]) > 1e-8) {
        for (auto la : batch_negLabels[d][j]) {
          auto row = RHSEmbeddings_->row(index(la));
...
float EmbedModel::trainNLLBatch(
  ...
  std::vector<std::vector<std::vector<Base>>> bnl(1);
  bnl[0] = batch_negLabels;

  backward(
      batch_exs, bnl,
      gradW, lhs, num_negs,
      rate0, labelRate, nRate, std::vector<unsigned>(0));
...
void EmbedModel::loadFeatEmb(const std::string& fname, const std::string sep) {
  cout << "-----------------\nReading k-mer embedding matrix from \'" << fname << "\'" << endl;
  ifstream ef(fname);
  string line;
  unsigned num_kmers = 0;
  for(unsigned lineNum = 1; getline(ef, line); lineNum++){
    vector<string> pieces;
    boost::split(pieces, line, boost::is_any_of(sep));

    if(pieces.size() > (args_->dim + 1)){
        cerr << "Truncating long (" << pieces.size() << ") record at line " << lineNum << endl;
        pieces.resize(args_->dim + 1);
    } else if(pieces.size() < (args_->dim + 1)){
        cerr << "Zero-padding short record at line " << lineNum << endl;
        auto zero = vector<string>(args_->dim + 1 - pieces.size(), "0.0");
        pieces.insert(pieces.end(), zero.begin(), zero.end());
    }

    auto idx = dict_->getId(pieces[0]);
    if(idx != -1 && dict_->getType(pieces[0]) == entry_type::word){
        num_kmers++;
        auto row = LHSEmbeddings_->row(idx);
        for(unsigned i = 0; i < args_->dim; i++){
            row(i) = boost::lexical_cast<Real>(pieces[i + 1].c_str());
        }
    }
  }
  cout << "Embedding of " << num_kmers << " k-mers was loaded from a previously trained model.\n-----------------\n" << endl;
  ef.close();
}
