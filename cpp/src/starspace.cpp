...
void StarSpace::initDataHandler() {
  if (args_->isTrain) {
    trainData_ = initData();
    trainData_->loadFromFile();
    // set validation data
    if (!args_->validationFile.empty()) {
      validData_ = initData();
      validData_->loadFromFile();
    }
  } else {
    if (args_->testFile != "") {
      testData_ = initData();
      testData_->loadFromFile();
    }
  }
}
...
shared_ptr<InternDataHandler> StarSpace::initData() {
  // if (args_->fileFormat == "fastText") {
    return make_shared<InternDataHandler>(args_, dict_, parser_);
  // } else if (args_->fileFormat == "labelDoc") {
  //   return make_shared<LayerDataHandler>(args_, dict_, parser_);
  // } else {
  //   cerr << "Unsupported file format. Currently support: fastText or labelDoc.\n";
  //   exit(EXIT_FAILURE);
  // }
...
void StarSpace::init() {
  ...
  if(args_->isCellSpace) dict_->CreateForCellSpace(parser_);
  else dict_->readFromFile(args_->trainFile, parser_);
  ...
  // if(args_->feat_emb != "") model_->loadFeatEmb(args_->feat_emb);
